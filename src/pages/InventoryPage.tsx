...
export default function InventoryPage() {
  const { state, removeInventoryItem, updateInventoryItem, initializeFridge } =
    useApp()
  const { colors } = useTheme()
  const { t } = useLanguage()
  const [showCamera, setShowCamera] = useState(false)
  const [showPopup, setShowPopup] = useState(false)
  const [activeTab, setActiveTab] = useState<'inventory' | 'planning'>(
    'inventory'
  )
                ...
                >
                  <div
                    style={{
                      ...
                      marginBottom: '24px',
                      animation:
                        'bounce 1s cubic-bezier(0.68, -0.55, 0.265, 1.55)',
                      animationDelay: '0.2s',
                  ...
                  {Object.entries(groupedInventory).map(
                    ([category, items], categoryIndex) => (
                      ...
                      >
                        <button
                          onClick={() =>
                            toggleCategory(category as ItemCategory)
                          }
                          style={{
                            width: '100%',
                            padding: '10px 12px',
                          ...
                          >
                            ...
                            <span
                              ...
                              style={{
                                ...
                                transform: collapsedCategories.has(
                                  category as ItemCategory
                                )
                                  ? 'rotate(0deg)'
                                  : 'rotate(180deg)',
                        ...
                        {!collapsedCategories.has(category as ItemCategory) && (
                          ...
                          >
                            {items.map((item) => (
                              ...
                              >
                                {/* Red delete area — behind the row, only used for swipe on narrow screens */}
                                {!isWide && (
                                  <div
                                    style={{
                                      position: 'absolute',
                                ...
                                {/* Tap target over revealed delete area (narrow screens only) */}
                                {!isWide && swipedItem === item.id && (
                                  <div
                                ...
                                <div
                                  onTouchStart={(e) =>
                                    !isWide && handleTouchStart(e, item.id)
                                  }
                                  onTouchEnd={(e) =>
                                    !isWide && handleTouchEnd(e, item.id)
                                  }
                                  onClick={() => {
                                    if (!isWide && swipedItem === item.id)
                                      setSwipedItem(null)
                                  }}
                                  style={{
                                    padding: '10px 12px',
                                    ...
                                    alignItems: 'center',
                                    transition: 'all 0.2s ease',
                                    backgroundColor: animatingItems.has(item.id)
                                      ? `${colors.primary}10`
                                      : 'transparent',
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <span
                                      style={{
                                        ...
                                        transform: animatingItems.has(item.id)
                                          ? 'scale(1.2)'
                                          : 'scale(1)',
                                      }}
                                    >
                                  ...
                                  >
                                    ...
                                    {/* Inline delete button — visible only on wide screens */}
                                    {isWide && (
                                      <button
                                        onClick={() =>
                                          removeInventoryItem(item.id)
