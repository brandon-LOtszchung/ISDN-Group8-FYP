...
export default function FridgeInitialization() {
  ...
  const handlePhotosCapture = async (photos: File[]) => {
    ...
    try {
      const items = await initializeInventory(photos)
  ...
  const handleConfirmItems = async () => {
    try {
      ...
      await dataService.addInventoryItems(detectedItems)
          ...
          <div className="text-center">
            ...
            <div className="bg-warm-50 rounded-xl p-5 mb-6 text-left">
              ...
              <div className="space-y-2" style={{ fontSize: '14px' }}>
                {['Take clear, well-lit photos', 'Capture different shelves/sections', 'Make sure labels are visible', 'Take 3-5 photos from different angles'].map((tip, i) => (
                  <div key={i} className="flex items-start gap-3 text-warm-700">
                    <span className="flex-shrink-0 w-6 h-6 bg-primary-500 text-white rounded-full flex items-center justify-center font-bold" style={{ fontSize: '12px' }}>{i + 1}</span>
                    <span>{tip}</span>
                  </div>
                ))}
              </div>
              ...
              {detectedItems.map((item, index) => (
                <div key={index} className="bg-white rounded-lg p-3 border border-warm-200">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => handleRemoveItem(index)} className="p-1 text-coral-500 hover:text-coral-700">
