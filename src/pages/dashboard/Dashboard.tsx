...
export default function Dashboard() {
  ...
  const mealTime = state.family?.preferences?.mealTimes?.[currentMealType as keyof typeof state.family.preferences.mealTimes]
      ...
      <div className="max-w-md mx-auto p-4 space-y-8">
        ...
        <div className="text-center py-6 animate-fade-in">
          ...
          <h1 className="text-3xl font-bold gradient-text mb-3 font-chinese whitespace-nowrap">
            {greeting === 'Good morning' ? '早晨！' : greeting === 'Good afternoon' ? '午安！' : '晚安！'}
          </h1>
          <p className="text-warm-600 text-lg font-medium font-chinese">
            {state.family?.name}
          </p>
          <div className="badge mt-3 font-chinese">
            準備{currentMealType === 'breakfast' ? '早餐' : currentMealType === 'lunch' ? '午餐' : '晚餐'}
          </div>
          {mealTime && (
        ...
        <div className="floating-card animate-slide-up">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <h2 className="section-title mb-0 font-chinese whitespace-nowrap">今晚邊個食飯？</h2>
          </div>
            ...
            {state.family?.members?.map((member) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-warm-800 font-chinese">{member.name}</p>
                    <p className="text-sm text-warm-600 font-chinese whitespace-nowrap">{member.age}歲</p>
                  </div>
        ...
        <div className="floating-card animate-slide-up">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              <span className="text-xl">📦</span>
              <h2 className="section-title mb-0 font-chinese whitespace-nowrap">雪櫃存貨</h2>
            </div>
              ...
              {Object.entries(groupedInventory).map(([category, items]) => (
                <div key={category}>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {items.map((item) => (
                      <div key={item.id} className="bg-gradient-to-br from-white/90 to-orange-50/80 backdrop-blur-sm rounded-2xl p-4 border border-white/60 shadow-sm hover:shadow-md transition-all duration-300">
        ...
        <div className="animate-slide-up">
          <Link to="/recipes" className="block">
            <Button className="w-full text-lg py-4 font-chinese whitespace-nowrap" disabled={selectedMembers.length === 0}>
